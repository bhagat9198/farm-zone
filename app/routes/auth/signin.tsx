...
export default function Signin() {
  ...
  useEffect(() => {
    (async() => {
      ...
      if(uid && utype) {
        const res = fetcher.submit({ uid, utype }, { action: '/auth/is-valid-user', method: 'post' })
        console.log('res :: ', res);
    ...
    <>
      <main className="contain py-16">
        <div className="max-w-lg mx-auto shadow px-6 py-7 rounded overflow-hidden">
          ...
          {
            userData && !userData.status && <p className="text-red-800 bg-red-100 py-2 px-10" >{userData.msg}</p>
          }
          <Form action="#" method="post" autoComplete="off">
            ...
            <div className="pt-4 my-2 flex justify-evenly items-center" >
              <p>You are :</p>
            ...
            <div className="mt-4">
              <button type="submit"
                className="block w-full py-2 text-center  bg-primary border border-primary rounded hover:bg-transparent hover:text-primary transition uppercase font-roboto font-medium">Login</button>
            </div>
          </Form>
          ...
          <p className="mt-4 text-center text-gray-600">Don't have account?
            <Link to="/auth/signup"
              className="font-bold text-primary">Register
              now</Link></p>
        </div>
      </main>
    </>
...
export const action = async (actionData) => {
  ...
  const userRes = await getUserDetails(authRes.data?.user?.id, values.registerAs);
  console.log('userRes :: ', userRes);
  if (!userRes.status) {
    return userRes;
  }

  return redirect(`/${values.registerAs}/profile/${authRes.data?.user?.id}`);
};
